(async () => {
  console.log('\nRunning integration tests against `worker.js` endpoints (script runner)...');
  ...
  if (agent.res.status !== 200) {
    console.error(`ERROR: /api/autonomous-agent returned ${agent.res.status}`);
    console.error('Response body:', agent.text);
    throw new Error('/api/autonomous-agent should return 200');
  }
  ...
  console.log('\nIntegration script runner completed successfully.');
})();
